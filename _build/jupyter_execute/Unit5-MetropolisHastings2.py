...
for i in tqdm(range(obs)):
    ...
    # acceptance ratio
    rr = (eta_prop/eta)**(beta - 1)*np.exp(alpha - alpha_prop - xi*(eta_prop - eta))         *np.exp(-alpha/alpha_prop - eta/eta_prop + alpha_prop/alpha + eta_prop/eta)         *data_prod**(alpha_prop - alpha) * prop_prod * ((alpha_prop*eta_prop)/(alpha*eta))**(n-1)
