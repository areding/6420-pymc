...
with pm.Model() as stressacids:
    # priors
    tau_nonsmokers = pm.Gamma('tau_nonsmokers', alpha=.0001, beta=.0001)
    sigma_nonsmokers = 1/pm.math.sqrt(tau_nonsmokers)
    mu_nonsmokers = pm.Normal('mu_nonsmokers', mu=0, sigma=100)

    tau_smokers = pm.Gamma('tau_smokers', alpha=.0001, beta=.0001)
    sigma_smokers = 1/pm.math.sqrt(tau_smokers)
    mu_smokers = pm.Normal('mu_smokers', mu=0, sigma=100)


    # likelihood
    plasma_aa_nonsmokers = pm.Normal('nonsmokers_aa', mu=mu_nonsmokers, 
                                   sd=sigma_nonsmokers, 
                                   observed=nonsmokers)
    plasma_aa_smokers = pm.Normal('smokers_aa', mu=mu_smokers, sd=sigma_smokers, 
                                observed=smokers)
